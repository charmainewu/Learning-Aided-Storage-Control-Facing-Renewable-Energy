...
def lstmmodel(neurons,layer,drop,batch_size,epochs,backday,n_outputs):
  ...
  print(neurons,layer,drop,batch_size,epochs,backday)
  model = Sequential()
  if layer == 1:
...
for n_outputs in [6,12,24]:
    ...
    for i in sample_without_replacement(len(param_com), n_iter, random_state=5):
      ...
      if result[count]["acc"] < acc_best:
        ...
        model.save("./data/windowprice"+str(n_outputs)+".h5")
      
      count = count + 1
    np.save('./data/windowprice_grid'+str(n_outputs)+'.npy', result) # 注意带上后缀名
