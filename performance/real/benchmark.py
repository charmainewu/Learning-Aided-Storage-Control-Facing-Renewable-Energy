...
class BenchmarkInAccurateDemand:
    ...
    def __init__(self, Capacity, Ntrain,Ntest,Nvalid, Backday, Interval, Price, Demand, Renewables, EndModel, PriceModel, WindModel,PriceScalar,WindScalar,WindStackData,PriceStackData,LSTM,DataSet):
      self.B = Capacity
      self.P = Price
      self.D = Demand
      self.R = Renewables
    ...
    def thb(self):
        ...
        for n in range(1, n_interval+1):
            ...
            #p_train = self.P[self.N+self.V+(n-2)*self.I:self.N+self.V+(n-1)*self.I]
            try:
            ...
            if xc>sum(d_pred):
              ...
              cost_thb[n-1],xc = self.Aour_demand(vb-d,d_real,p_real,xc)
    ...
    def deta(self):
        ...
        for n in range(1, n_interval+1):
            ...
            if xc>sum(d_pred):
              ...
              x,d,va,vb,cost_shot = self.Aour_gmm(d_pred,p_real,clf,0)
              cost_deta[n-1],xc = self.Aour_demand(vb-d,d_real,p_real,xc)
    ...
    def Aour_demand(self,delta,a,p,xc):
        ...
        for t in range(T):
          delta_legal =  min(max(max(-xc,-a[t]),delta[t]),self.B)
        ...
        for t in range(T):
            if t == 0 :
                x[t] = min(max(0.0,delta[t]),self.B)
                vb = x[t]
                va = a[t]
                cost = cost + (va+vb)*p[t]
            else:
    ...
    def leta(self):
        ...
        for n in range(1, n_interval+1):
            ...
            abar, ts, tnz = self.isDecompose(d_pred,0)
            delta = self.Aour_pred_transfer(abar,ts,tnz,(n-1)*self.I,(n)*self.I)
            cost[n-1],xc = self.Aour_demand(delta,d_real,p_real,xc)
    ...
    def etoe(self,t):
        x = self.Dt[self.N+self.V+t-self.BD[0]:self.N+self.V+t,:]
        x = np.array(x); x = np.reshape(x, (1,x.shape[0],x.shape[1]))
        
        x = np.swapaxes(x,0,1)
        x = torch.from_numpy(x).to(torch.float32)

        model = self.ED.eval() # 转换成测试模式
        val_x = Variable(x)
        val_out = model(val_x)
        return val_out
    ...
    def eeta(self):
        ...
        for n in range(1, n_interval+1):
            ...
            print(p_real)
            """
            cost[n-1],xc = self.Aour_demand(delta[0]*10000,d_real,p_real,xc)
            """
            print("eeta_cost")
            print(cost)
            print(xc)
    ...
    def nos(self):
        ...
        for n in range(1, n_interval+1):
            ...
            cost[n-1],xc = self.Aour_demand(np.zeros(len(d_real)),d_real,p_real,0)
            
            """
            print("nos_action")
            print(np.zeros(len(d_real)))
            """
