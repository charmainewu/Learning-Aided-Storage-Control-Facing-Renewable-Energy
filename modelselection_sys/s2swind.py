# -*- coding: utf-8 -*-
"""
Created on Tue Jan 26 15:46:56 2021
...
def lstmmodel(neurons,layer,drop,batch_size,epochs,backday,n_outputs):
  ...
  model.fit(x_train, y_train, epochs=epochs, batch_size = batch_size, validation_data=(x_valid, y_valid), verbose=0, callbacks=[early_stopping])
...
for n_outputs in [6,12,24]:
    ...
    for i in sample_without_replacement(len(param_com), n_iter, random_state=5):
      ...
      if result[count]["acc"] < acc_best:
        ...
        model.save('./data/s2swind'+str(n_outputs)+'.h5')
      count = count + 1
    np.save('./data/s2swind_grid'+str(n_outputs)+'.npy', result) # 注意带上后缀名
